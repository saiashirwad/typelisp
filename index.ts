...
type numPattern<digit extends number, rest extends string> = `${digit}${rest}`;

type shiftNumber<s extends State<any>> = s["unscanned"] extends numPattern<
	infer digit,
	infer rest
>
	? State<
...
type shiftSymbol<s extends State<any>> =
	s["unscanned"] extends `${infer char}${infer _}`
		? isDelimiter<char> extends true
			? s
			: accumulateSymbol<s["unscanned"]> extends Accumulated<
						infer sym,
						infer remaining
					>
...
type shiftString<s extends State<any>> =
	s["unscanned"] extends `"${infer content}"${infer rest}`
		? State<trimLeft<rest>, appendToken<s, "string", content>, s["stack"]>
		: s;

type consumeChar<s extends State<any>, ch extends string | number> = trimLeft<
...
type extractBindings<T extends Token[]> = T extends [
	List<List2<infer name extends Token, infer value extends Token>>,
...
type eval<tok extends Token, env extends Environment> = tok extends Num<any>
	? tok
	: tok extends Sym<infer sym>
		? lookupEnv<env, sym>
		: tok extends List<matchList<Token, infer first, infer rest>>
			...
			: tok extends Def<infer n, infer val>
				? extendEnv<
					...
					>
				: tok extends Let<
							infer bindings extends Array<LetBinding<any, any>>,
							infer body
						>
					? evalLet<bindings, body, env>
					: never;

type ApplicationArgs<a extends Token, b extends Token> = [a, b];
...
type result2 = eval<parse<initialState<"(sub 6 (add 3 2))">>[0], initialEnv>;
